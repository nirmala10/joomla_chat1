...
{   var getUrl = window.location;
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = baseUrl+'/plugins/content/joomla_chat/bundle/jquery/css/base/jquery-ui.css';
    link.media = 'all';
...
if (!document.getElementById(cssId)){
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = baseUrl+'/plugins/content/joomla_chat/css/jquery.ui.chatbox.css';
    link.media = 'all';
    head.appendChild(link);
}
...
jQuery(function($){
     ...
     if(localStorage.getItem('init') != 'false'){
         ...
         if (browserSupportsLocalStorage() == false)  {
             // check browser for local storage
             alert(lang.sterror);
             return;
         }
         ...
         $(document).on("connectionclose", function(e){
             $("#user_list .inner_bt #usertab_icon").css({'background': 'url(plugins/content/joomla_chat/images/offline.png)no-repeat top left'});
